...
from config.language import SEND_MESSAGE_ALL, SEND_MESSAGE_ALL_CONFIRMATION, SEND_MESSAGE_ALL_SENT, SEND_MESSAGE_ALL_DENIED
...
@router.message(lambda message: message.text.casefold()==SEND_MESSAGE_ALL.casefold() and message.from_user.id in [int(user) for user in SUPERUSERS])
